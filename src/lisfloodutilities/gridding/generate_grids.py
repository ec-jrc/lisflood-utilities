...
def get_grid(grid_utils: GriddingUtils, filename: Path,
             tiff_filepath: Optional[Path]=None, get_existing_tiff: bool=False) -> np.ndarray:
    grid_data = None
    # read the existing TIFF instead of interpolating the grid 
    if get_existing_tiff and tiff_filepath is not None and tiff_filepath.exists():
        try:
            grid_data = grid_utils.read_tiff(tiff_filepath)
        except Exception as e:
            print_msg(f"Warning: The file {tiff_filepath} is probably missing or is corrupt. "
                      f"It will be regenerated by this tool.")
...
def run(config_filename: str, infolder: str, output_file: str, processing_dates_file: str, file_utils: FileUtils,
        ...
        start_date: Optional[datetime] = None, end_date: Optional[datetime] = None, interpolation_mode: str = 'adw',
        use_broadcasting: bool = False, memory_save_mode: str = '0') -> None:
    """
    ...
    output_writer_tiff = None
    output_writer_netcdf = None
