...
from lisfloodutilities.gridding.lib.utils import Printable, Config
...
class NetCDFWriter(OutputWriter):
    ...
    def __init__(self, conf: Config, overwrite_file: bool = False, quiet_mode: bool = False):
        ...
        self.scale_factor = float(self.conf.get_config_field('PROPERTIES', 'VALUE_SCALE'))
        self.add_offset = float(self.conf.get_config_field('PROPERTIES', 'VALUE_OFFSET'))
    ...
    def write_timestep(self, grid: np.ndarray, timestep: int = -1):
        if timestep >= 0:
            ...
            values = grid
            values[values!=self.conf.VALUE_NAN] -= self.add_offset
            values[values!=self.conf.VALUE_NAN] *= self.scale_factor
            # values = self.setNaN(grid)
            if self.unit_conversion != 1.0:
                values = values * self.unit_conversion
            values[np.isnan(values)] = (self.conf.VALUE_NAN - self.add_offset) * self.scale_factor
    ...
    def __setup_netcdf_metadata(self, start_date: datetime = None):
        # General Attributes
        time_created = timex.ctime(timex.time())
        self.nf.history = f'Created {time_created}'
        self.nf.Conventions = self.NETCDF_CONVENTIONS
        self.nf.Source_Software = self.conf.get_config_field('GENERIC','NETCDF_SPHEREMAP_VERSION')
        self.nf.reference = self.conf.get_config_field('GENERIC','NETCDF_REFERENCE')
        self.nf.title = self.conf.get_config_field('GENERIC','NETCDF_TITLE')
        self.nf.keywords = self.conf.get_config_field('GENERIC','NETCDF_KEYWORDS')
        self.nf.source = self.conf.get_config_field('GENERIC','NETCDF_SOURCE')
        self.nf.institution = self.conf.get_config_field('GENERIC','NETCDF_INSTITUTION')
        self.nf.comment = self.conf.get_config_field('GENERIC','NETCDF_COMMENT')
...
#         nrows = int(self.conf.get_config_field('DIMENSION','ROWS'))
        ...
        var_std_name = self.conf.get_config_field('PROPERTIES', 'STANDARD_NAME')
        ...
        value.standard_name = var_std_name
        value.long_name = self.conf.get_config_field('PROPERTIES', 'LONG_NAME')
        value.cell_methods = self.conf.get_config_field('PROPERTIES', 'CELL_METHODS')
        value.units = self.conf.get_config_field('PROPERTIES', 'UNIT')
        value.valid_min = int(value_min / self.scale_factor)
        if value_max != self.conf.VALUE_NAN:
            value.valid_max = int(value_max / self.scale_factor)
        value.scale_factor = self.scale_factor
        value.add_offset = self.add_offset
        ...
        self.print_msg(f'var-name: {var_std_name}')
...
class GDALWriter(OutputWriter):
    ...
    def write_timestep(self, grid: np.ndarray, timestep: int = -1):
        ...
        if self.overwrite_file or not self.filepath.is_file():
            ...
            ds = self.driver_gtiff.Create(str(self.filepath), size_lons, size_lats, 1, gdal.GDT_Int16)
            ...
            # Write the band
            ds.GetRasterBand(1).SetNoDataValue(self.conf.VALUE_NAN) # optional if no-data transparent
