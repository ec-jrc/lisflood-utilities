...
# Large rivers shapefile (http://ihp-wins.unesco.org/layers/geonode:world_rivers)
rivers_shp = gpd.read_file(os.path.join(world_rivers_dir,'world_rivers.shp'))
...
catchment_dirs = glob.glob(os.path.join(database_dir,'*_*',""))
...
for ii in np.arange(len(catchment_dirs)):
    ...
    np.savetxt(os.path.join(corrected_locations_dir,ID+'.txt'), np.array([StatLatCorr,StatLonCorr]), delimiter=',',fmt='%1.5f')
