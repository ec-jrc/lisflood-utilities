...
def main():
    ...
    for scenario in scenarios:
        for yr in yrs:
            ...
            vcf_path = glob.glob(os.path.join(config['vcf_folder'],'*2015001*.tif'))[0]
    ...
    for year in years:
        ...
        for month in np.arange(1,13):
            ...
            vcf_path = glob.glob(os.path.join(config['vcf_folder'],'*'+str(vcf_year)+'001*.tif'))[0]
            print('Loading MEaSUREs VCF data ('+vcf_path+')')
            src = rasterio.open(vcf_path)
            fracforest = src.read(1).astype(np.double)
            fracforest = (fracforest/100).clip(0,1)
            ...
            hyde_garea_cr = np.array(pd.read_csv(os.path.join(config['hyde_folder'],'general_files','garea_cr.asc'), 
            ...
            hyde_ir_norice = np.array(pd.read_csv(os.path.join(config['hyde_folder'],'baseline','zip','ir_norice'+str(hyde_year)+'AD.asc'), 
            ...
            hyde_ir_rice = np.array(pd.read_csv(os.path.join(config['hyde_folder'],'baseline','zip','ir_rice'+str(hyde_year)+'AD.asc'), 
            ...
            print('Saving data to in Numpy format (folder '+config['output_folder']+')')
            t0 = time.time()
