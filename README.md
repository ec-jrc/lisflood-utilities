...
1. The water fraction at high latitudes was based on HILDA+ V1.0 (1-km resolution; 1900–2019; [Winkler et al., 2021](https://doi.org/10.1038/s41467-021-22702-2)). The data are available via the [PANGAEA data repository](https://doi.org/10.1594/PANGAEA.921846).
1. The forest fraction was derived from the MEaSUREs Vegetation Continuous Fields (VCF) dataset (0.05° resolution; 1982–2016; [Song et al., 2018](https://doi.org/10.1038/s41586-018-0411-9)) available from the [NASA Earthdata website](https://doi.org/10.5067/MEaSUREs/VCF/VCF5KYR.001).
...
1. Loads and resamples the global 1-km GAIA data to the template map resolution and calculates the sealed fraction.
1. Loads and resamples the global 1-km GSWE data to the template map resolution and calculates the water fraction.
1. Loads and resamples the global 1-km HILDA+ data to the template map resolution and and fill gaps in the GSWE water fraction map.
...
1. Loads and resamples the global 0.05° MEaSUREs VCF data and calculates the forest fraction.
1. Loads and resamples the global 0.083° HYDE data and calculates the irrigation (no rice) and irrigated rice fractions.
...
The script can be run on a normal desktop PC (Windows and Linux) with 16 GB or more physical memory.
