...
def main():
    if os.path.isdir(os.path.join(config['gaia_folder'],'geotiffs'))==False:
        os.makedirs(os.path.join(config['gaia_folder'],'geotiffs'))
        
    if os.path.isdir(os.path.join(config['gaia_folder'],'netCDF_1km'))==False:
        os.makedirs(os.path.join(config['gaia_folder'],'netCDF_1km'))

    if config['use_proxy']==True:
        proxy_credentials = pd.read_csv('proxy_credentials.cfg',header=None,index_col=False)
        
    # Load GAIA shapefile
    GAIA_shape = gpd.read_file(os.path.join(config['gaia_folder'],'GAIA_shape','GAIA_nameID_1deg.shp'))
    ...
    for ii in np.arange(GAIA_shape.shape[0]):    
        ...
        if os.path.isfile(os.path.join(config['gaia_folder'],'geotiffs',filename)): continue
        if config['use_proxy']==True:
            ...
            proxy_string = ''
        print('===============================================================================')
        command = 'wget '+proxy_string+url_pre+filename+' --no-clobber --directory-prefix='+os.path.join(config['gaia_folder'],'geotiffs')
    ...
    geotiff_files = glob.glob(os.path.join(config['gaia_folder'],'geotiffs','*.tif'))
    for jj in np.arange(len(years)):
        ...
        save_netcdf(
            file=os.path.join(config['gaia_folder'],'netCDF_1km',str(year)+'.nc'), 
